...
func TestPreparedQuery_Execute_Failover(t *testing.T) {
	...
	// Set up a bare-bone query with failover enabled
	...
	// Set up a bare-bone query with failover enabled
	// with list of non-existent dc then dc3 (0 node) and finally dc2 (2 nodes)
	query = structs.PreparedQueryRequest{
		Datacenter: "dc1",
		Op:         structs.PreparedQueryCreate,
		Query: &structs.PreparedQuery{
...
func TestPreparedQuery_queryFailover(t *testing.T) {
	...
	t.Run("Failover on dc1 returned no nodes", func(t *testing.T) {
		...
		query.Service.Failover.SkipLocalDatacenter = true
		{
			...
			require.Equal(t, "", mock.JoinQueryLog())
		}
	})

	t.Run("failover on dc2 works", func(t *testing.T) {
		...
		query.Service.Failover.SkipLocalDatacenter = true
		{
			...
			require.Equal(t, "dc2:PreparedQuery.ExecuteRemote", mock.JoinQueryLog())
		}
	})

	t.Run("Failover on dc3 works", func(t *testing.T) {
		...
		query.Service.Failover.SkipLocalDatacenter = true
		{
			...
			require.Equal(t, nodes(), reply.Nodes)
			require.Equal(t, 3, reply.Failovers)
			require.Equal(t, "dc2:PreparedQuery.ExecuteRemote|dc3:PreparedQuery.ExecuteRemote", mock.JoinQueryLog())
		}
	})
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			query.Service.Failover.SkipLocalDatacenter = false
